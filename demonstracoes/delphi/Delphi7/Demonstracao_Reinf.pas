unit Demonstracao_Reinf;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes,
  Graphics, Controls, Forms, Dialogs, ReinfClientX_TLB, StdCtrls,
  ExtCtrls, ComCtrls;
...
begin
  ...
  cbambiente.ItemIndex := 1; // 0 - Produção, 1 - homologação
end;

procedure TfrmReinf.CarregarConfiguracao;
begin
...
begin
  ...
  Reinf.DiretorioLogErro    := ExtractFilePath(ParamStr(0))  + 'LogErro\';
  Reinf.DiretorioLog        := ExtractFilePath(ParamStr(0))  + 'Log\';
...
begin
  ...
  xml.SaveToFile(ExtractFilePath(ParamStr(0)));
